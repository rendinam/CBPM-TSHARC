...
int cbpm_dump_summ_buf( FILE *fp, int iidx ) {

  CBPM_DATA *dp;
  dp = CTL_System.p_Module[iidx]->dsp_data;

  fprintf(fp, " Communication Structure Dump for: cbpm_summ_buf\n");
  int  v0,  v1,  v2,  v3,  v4,  v5,  v6;
  int sv0, sv1, sv2, sv3, sv4, sv5, sv6;

  fprintf(fp, "data:\n");
  for (v0 = 0; v0 < CBPM_MAX_AUTO_SAMPLES; v0++) {


    fprintf(fp, "gain:\n");
    for (sv0 = 0; sv0 < CBPM_MAX_CARDS; sv0++) {
      fprintf(fp, "%d  ", dp->cbpm_summ_buf.data[v0].gain[sv0] );
    }
    fprintf(fp, "\n");
    fprintf(fp, "delay: %d  \n", dp->cbpm_summ_buf.data[v0].delay);
    fprintf(fp, "chan_delay: %d  \n", dp->cbpm_summ_buf.data[v0].chan_delay);
    fprintf(fp, "signal:\n");
    for (sv0 = 0; sv0 < CBPM_MAX_CARDS; sv0++) {
      fprintf(fp, "%f  ", dp->cbpm_summ_buf.data[v0].signal[sv0] );
    }
    fprintf(fp, "\n");
  }
  fprintf(fp, "\n");

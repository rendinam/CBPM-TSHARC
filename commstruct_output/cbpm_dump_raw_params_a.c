...
int cbpm_dump_raw_params( FILE *fp, int iidx ) {

  CBPM_DATA *dp;
  dp = CTL_System.p_Module[iidx]->dsp_data;

  fprintf(fp, " Communication Structure Dump for: cbpm_raw_params\n");
  int  v0,  v1,  v2,  v3,  v4,  v5,  v6;
  int sv0, sv1, sv2, sv3, sv4, sv5, sv6;

  fprintf(fp, "num_turns: %d  \n", dp->cbpm_raw_params.num_turns);
  fprintf(fp, "scale_mode: %d  \n", dp->cbpm_raw_params.scale_mode);
  fprintf(fp, "trig_turns_delay: %d  \n", dp->cbpm_raw_params.trig_turns_delay);
  fprintf(fp, "spacex_turn: %d  \n", dp->cbpm_raw_params.spacex_turn);
  fprintf(fp, "use_data_enable: %d  \n", dp->cbpm_raw_params.use_data_enable);
  fprintf(fp, "trig_mask: %d  \n", dp->cbpm_raw_params.trig_mask);
  fprintf(fp, "phase_race_wait: %d  \n", dp->cbpm_raw_params.phase_race_wait);

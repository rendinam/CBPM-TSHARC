...
int cbpm_dump_op_gain( FILE *fp, int iidx ) {

  CBPM_DATA *dp;
  dp = CTL_System.p_Module[iidx]->dsp_data;

  fprintf(fp, " Communication Structure Dump for: cbpm_op_gain\n");
  int  v0,  v1,  v2,  v3,  v4,  v5,  v6;
  int sv0, sv1, sv2, sv3, sv4, sv5, sv6;

  fprintf(fp, "active_gain_settings:\n");
  for (v0 = 0; v0 < CBPM_MAX_CARDS; v0++) {
    for (v1 = 0; v1 < CBPM_MAX_TIMING_BLOCKS; v1++) {
      fprintf(fp, "%d  ", dp->cbpm_op_gain.active_gain_settings[v0][v1] );
    }
    fprintf(fp, "\n");
  }
  fprintf(fp, "\n");

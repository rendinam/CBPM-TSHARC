//------------------------------------------------------------------------
// File         :  cbpm_dump_adc_buffer5_a.c
...
int cbpm_dump_adc_buffer5( FILE *fp, int iidx ) {

  CBPM_DATA *dp;
  dp = CTL_System.p_Module[iidx]->dsp_data;

  fprintf(fp, " Communication Structure Dump for: cbpm_adc_buffer5\n");
  int  v0,  v1,  v2,  v3,  v4,  v5,  v6;
  int sv0, sv1, sv2, sv3, sv4, sv5, sv6;

  fprintf(fp, "raw_data:\n");
  for (v0 = 0; v0 < CBPM_ADC_BUF_USE; v0++) {
    fprintf(fp, "%d  ", dp->cbpm_adc_buffer5.raw_data[v0] );
  }
  fprintf(fp, "\n");

//------------------------------------------------------------------------
// File         :  cbpm_dump_test_results_a.c
...
int cbpm_dump_test_results( FILE *fp, int iidx ) {

  CBPM_DATA *dp;
  dp = CTL_System.p_Module[iidx]->dsp_data;

  fprintf(fp, " Communication Structure Dump for: cbpm_test_results\n");
  int  v0,  v1,  v2,  v3,  v4,  v5,  v6;
  int sv0, sv1, sv2, sv3, sv4, sv5, sv6;

  fprintf(fp, "EncodedStatus:\n");
  for (v0 = 0; v0 < CBPM_NUM_TESTS_ENCODED; v0++) {
    fprintf(fp, "%d  ", dp->cbpm_test_results.EncodedStatus[v0] );
  }
  fprintf(fp, "\n");
  fprintf(fp, "Statuses:\n");
  for (v0 = 0; v0 < CBPM_NUM_TESTS; v0++) {
    for (v1 = 0; v1 < CBPM_MAX_ACQ_CONTROLLERS; v1++) {
      fprintf(fp, "%d  ", dp->cbpm_test_results.Statuses[v0][v1] );
    }
    fprintf(fp, "\n");
  }
  fprintf(fp, "\n");

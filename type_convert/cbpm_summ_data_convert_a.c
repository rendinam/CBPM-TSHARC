...
//------------------------------------------------------------------------
// File         :  CBPM_SUMM_DATA_convert_a.c
...
int * cbpm_summ_data_convert(int mode, int *p_buf, int dtype, int dcount,
...
//-----------------------------------------------------------------------+
   ...
   char *func_name = "cbpm_summ_data_convert";

   int i;
   CBPM_SUMM_DATA *p_cbpm_summ_data = NULL;
   void *p_sub_data;
   ...
   //-----------------------------------------------------------------------+
   if (dtype == CBI_STRUCT) {
      p_cbpm_summ_data = ((CBPM_SUMM_DATA *) p_data + dcount * offset);
   } else {
   ...
   for (i = 0; i < dcount; i++) {
      ...
      p_sub_data = (void *) &(p_cbpm_summ_data->gain);
      count      = sizeof p_cbpm_summ_data->gain / sizeof(int);
      p_buf      = cbi_int_convert(mode,p_buf,CBI_INT,count,0,p_sub_data);

      p_sub_data = (void *) &(p_cbpm_summ_data->delay);
      count      = sizeof p_cbpm_summ_data->delay / sizeof(int);
      p_buf      = cbi_int_convert(mode,p_buf,CBI_INT,count,0,p_sub_data);

      p_sub_data = (void *) &(p_cbpm_summ_data->chan_delay);
      count      = sizeof p_cbpm_summ_data->chan_delay / sizeof(int);
      p_buf      = cbi_int_convert(mode,p_buf,CBI_INT,count,0,p_sub_data);

      p_sub_data = (void *) &(p_cbpm_summ_data->signal);
      count      = sizeof p_cbpm_summ_data->signal / sizeof(float);

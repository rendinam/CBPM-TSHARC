...
//------------------------------------------------------------------------
// File         :  CBPM_CHAN_CAL_DATA_convert_a.c
...
int * cbpm_chan_cal_data_convert(int mode, int *p_buf, int dtype, int dcount,
...
//-----------------------------------------------------------------------+
   ...
   char *func_name = "cbpm_chan_cal_data_convert";

   int i;
   CBPM_CHAN_CAL_DATA *p_cbpm_chan_cal_data = NULL;
   void *p_sub_data;
   ...
   //-----------------------------------------------------------------------+
   if (dtype == CBI_STRUCT) {
      p_cbpm_chan_cal_data = ((CBPM_CHAN_CAL_DATA *) p_data + dcount * offset);
   } else {
   ...
   for (i = 0; i < dcount; i++) {
      ...
      p_sub_data = (void *) &(p_cbpm_chan_cal_data->data);
      count      = sizeof p_cbpm_chan_cal_data->data / sizeof(float);
      p_buf      = cbi_float_convert(mode,p_buf,CBI_FLOAT,count,0,p_sub_data);

      p_sub_data = (void *) &(p_cbpm_chan_cal_data->error);
      count      = sizeof p_cbpm_chan_cal_data->error / sizeof(float);
      p_buf      = cbi_float_convert(mode,p_buf,CBI_FLOAT,count,0,p_sub_data);

      //Increment the data pointer
      p_cbpm_chan_cal_data++;
   }
